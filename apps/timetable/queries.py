...
def get_order_objects(db: Session, cort_id=None):
    ...
    result = db.query(user_models.Order, starttime_table, endtime_table).join(starttime_table, user_models.Order.starttime == starttime_table.id).join(endtime_table, user_models.Order.endtime == endtime_table.id)
    if (cort_id):
