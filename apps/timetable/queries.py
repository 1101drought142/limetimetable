...
def get_order_objects(db: Session, cort_id=None):
    ...
    result = db.query(user_models.Order, starttime_table, endtime_table, user_models.Client) \
...
def get_order_object(db: Session, id: int):
    ...
    starttime_table = aliased(user_models.TimeIntervalObjects)
    endtime_table = aliased(user_models.TimeIntervalObjects)
    return db.query(user_models.Order, starttime_table, endtime_table, user_models.Client).join(starttime_table, user_models.Order.starttime == starttime_table.id).join(endtime_table, user_models.Order.endtime == endtime_table.id).join(user_models.Client, user_models.Order.client == user_models.Client.id).filter(user_models.Order.id == id).filter( (user_models.Order.payed==True) | (user_models.Order.time_created > last_ten_minutes)  | (user_models.Order.created_by_admin==True) ).first()
...
def create_new_object(db: Session, date: datetime.date, starttime: datetime.time, endtime: datetime.time, payed: bool, client_name:str|None, client_phone: str|None, client_mail: str|None, bitrix_id: str|None, site_id: str|None, cort_id:int, created_by_admin = False, color=None):
    ...
    if not(site_id):
        ...
        client_id = site_id
    new_order = user_models.Order( date=date, starttime=time_start.id, endtime=time_end.id, payed=payed, client=int(client_id), cort=cort_id, created_by_admin=created_by_admin, color=color)
    db.add(new_order)
    db.commit()
    if (new_order.id):
        return new_order.id
...
def get_today_intervals(db: Session) -> list:
    current_time = datetime.datetime.now().time()
    return db.query(user_models.TimeIntervalObjects).filter(user_models.TimeIntervalObjects.time_object > current_time).all()
